/* glyph_cursor.c — shared save-under arrow cursor (see glyph_cursor.h).
 *
 * Renders a procedurally-built arrow sprite (white fill, black outline, soft
 * drop shadow) into a caller-supplied surface, saving the pixels underneath so
 * they can be restored before the caller repaints. Singleton state — one cursor
 * per process, which is all a compositor or a greeter needs. */
#include "glyph_cursor.h"
#include <string.h>

#define CW GLYPH_CURSOR_W
#define CH GLYPH_CURSOR_H

/* Pre-rendered ARGB cursor sprite (A=0 transparent, A=FF opaque). */
...
void glyph_cursor_init(surface_t *fb)
{
    s_fb = fb;
    s_visible = 0;
    if (!s_sprite_ready)
